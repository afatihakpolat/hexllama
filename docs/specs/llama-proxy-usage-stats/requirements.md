...
- Allow the user to define app-wide input, cache, and output token rates and view derived cost analysis across the same live and historical stats surfaces.
...
- Adding reporting/export, quotas, or invoice-grade billing workflows.
...
- A desktop user who wants to define pricing assumptions and inspect derived costs per session, template, and time window.
...
- A user defines uncached-input, cache, and output token rates, then compares estimated cost totals by session, template, and recent requests.
...
- The app must persist app-wide cost settings for uncached input, cache, output, and display currency.
- The renderer must expose a dedicated cost-analysis tab inside Usage Stats where the user can edit those rates and view derived cost totals for summary, sessions, grouped sessions, templates, days, and recent requests.
...
- Cost analysis may be derived from current token rollups at read time; it does not need immutable historical pricing snapshots in the first implementation.
...
- Cost settings should be app-wide and durable across restarts.
...
- The user can define uncached-input, cache, and output rates plus a currency label, close and reopen the app, and see the same rates reused in the Cost tab.
- The Cost tab shows derived totals for the current filter window across overall summary, per-session rows, grouped session analysis, templates, days, and recent requests.
- Changing the configured rates recalculates displayed cost analysis for existing persisted history without requiring a migration.
...
- Is repricing historical sessions with the latest configured rates acceptable, or will users later need immutable historical price snapshots?